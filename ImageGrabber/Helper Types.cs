  ...
  public class ImageInfo : IDisposable {
      ...
      get {
        if (_image == null) {
          if (_sourceObject is Icon)
            ...
            using (Graphics grfx = Graphics.FromImage(_image))
    ...
    public static void Image_Converter(Image bmp, out System.Windows.Controls.Image image) {
      using (var bitmap = new Bitmap(bmp)) {
        ...
        image = new System.Windows.Controls.Image {Source = bitmapSource};
    ...
    private static Image ScaleImage(BitmapSource bitmapSource, Size scale) {
      ...
      double newWidthRatio = (scale.Width != -1 ? scale.Width/(double) bitmapSource.PixelWidth : 1.0);
      double newHeightRatio = (scale.Height != -1 ? ((scale.Width*bitmapSource.PixelHeight)/(double) bitmapSource.PixelWidth)/bitmapSource.PixelHeight : 1.0);
